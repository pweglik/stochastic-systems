...
class CartPole:
    ...
    def reset(self):
        ...
        self._pole_angle = (initial_pole_angle + math.pi) % (
            2 * math.pi
        ) - math.pi  # (rad)
    ...
    def step(self, action: float):
        ...
        self._state = [
            ...
            self._state[0] < -self.x_threshold
            or self._state[0] > self.x_threshold
            or self._state[2] < -self.theta_threshold
            or self._state[2] > self.theta_threshold
            or self._step == 500
        )
        self._done = bool(done)
        return self._state, 1.0, self._done, self._done, {}
